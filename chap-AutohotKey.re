...
AutoHotkeyは、ショートカットキーを自分で作成するなど、キーボードをカスタマイズできるスクリプトエンジンです（。トリガーとなるキーの組み合わせや単打、トリガーした後の処理を簡単な文法で設定できます。キーボードのキーの入力を別のキーに変更するだけでなく、文字列操作やファイル操作、マウス操作、GUIプログラムの作成、特定ウィンドウでのみ動作、タイマーなど多彩なコマンドが用意されています。今回の本では、簡単に設定できるトリガーのキーの組み合わせでキー入力、マウス操作を行う方法を紹介します。文字列操作やファイル操作、GUI操作、別ソフトの起動などをしたい方は英語ですがAutoHotKeyの公式リファレンス、日本語がいいならAutoHotKey Wikiを参照してください。ただし、AutoHotKey Wikiは2014年で更新が終わっているため、注意が必要です。調べれば、解説しているWebサイトもあります。
...
しかしこれでは、修飾キーとの同時押ししか定義できません（例：aとbの同時押しが定義できない）。そこで、Send関数を使います。
...
//image[ChangeKeyafter][変更後][scale=1]

設定を増やしていくと1つのファイルにすべて書くのではなく分割したいです。その時、ahkファイルをひとつずつ実行しているとインジケーターが雑多になるし、設定OFFにしたいときにひとつずつ終了するのは手間です。1つのahkファイルに、他のahkファイルを起動する設定を書きましょう。
...
//image[ahkStudio][AHK-Studio]

== ホットストリング
ホットストリングは、指定した文字列が入力されたあと、終了文字が入力されるとその文字列を削除して設定した文字列を入力するというものです。つまり、置換ですね。デフォルトの終了文字は、-()[]{}':;"/\,.?!{Enter}{Space}{Tab}です。追加することもできますし、終了文字を押さなくとも置換を実行することもできます。大文字小文字は区別しません。
...
終了文字を押さずとも置換を実行するには次のとおりです。

//emlist[ホットストリングの例（終了文字なし）]{
:*:kita-::
Clipboard = ｷﾀ━━━━━━(ﾟ∀ﾟ)━━━━━━ !!!!!
Send,^v
Return
...
最初の::の間にオプションを設定します。*を書くことで終了文字なしで置換を実行します。日本語環境でホットストリングを使用する注意点としては、次のものがあります。
